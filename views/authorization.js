define([
  ...
  if (localStorage.getItem('tokenId')) {
    _accessToken = localStorage.getItem('tokenId');
    _accessSecret = localStorage.getItem('secret');

    Access.token = localStorage.getItem('tokenId');
    Access.secret = localStorage.getItem('secret');
  }
  ...
  var Utils = {
     ...
     prepareXAuthData: function(baseParams, options) {
      ...
      makeAuthHeaderFromParamsObject: function(authParamsObject) {
        var paramArray = _.map(authParamsObject,
      ...
      signRequest: function(params, options) {
        var signature = null;
        if (!options.requestMethod) options.requestMethod = 'POST';
        ...
        switch (params.oauth_signature_method) {
          ...
          case 'HMAC-SHA1':
            ...
            var queryParams = this.getQueryParamsFromURL(options.url);
            if (_.size(queryParams) > 0) {
              allParams = _.extend(allParams, queryParams);
            }
            ...
            if (options.tokenSecret) {
              key += this.encode(options.tokenSecret);
            }
            ...
            this.debug('NormalizedParams ', normalizedParams);
            this.debug('ConsumerSecret ', options.consumerSecret);
            if (options.tokenSecret) this.debug('tokenSecret ', options.tokenSecret);
            this.debug('Key ', key);
      ...
      normalizeParams: function(paramDict) {
        var sortedParamKeys = _.sortBy(_.keys(paramDict), function(key) { return key; });
      ...
      constructBaseString: function(method, uri, params) {
        return [ method.toUpperCase(), this.encode(uri), this.encode(params) ].join('&');
      },
      ...
      getQueryParamsFromURL: function(theURL) {
        var params = {};
    ...
    // Minimum set of functions for OAuth Sigining.
    var OAuth = {
  ...
  return {
    getSignature: function(url, method) {
      ...
      // If its not a login call and we have the tokens we
      // need then need to include them now.

      if (includeToken && accessToken && accessSecret) {
        authParams.oauth_token = accessToken;
        authSigningOptions.tokenSecret = accessSecret;
      }

      var authParamsObject = Utils.prepareXAuthData(authParams, authSigningOptions);

      return Utils.makeAuthHeaderFromParamsObject(authParamsObject);
    }
  }
  }
